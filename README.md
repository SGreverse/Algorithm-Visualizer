...
This project is built on a custom producer-consumer threading model. The UI and rendering run cleanly on the main thread, while the algorithms execute on an isolated background worker thread. They communicate via standardized VTable hooks (`compare`, `swap`, `write`) backed by mutexes and condition variables. 

This architecture allows for real-time visual updates and live operation without ever bottlenecking the render loop.
...
*   **OOP in c:** If you want to learn about the techniques i used for creating pseudo-generic OOP c you can check out my other repo: [OOP In C](https://github.com/SGreverse/OOP-in-C)
## How to Use
...
*   **Algorithm Navigation:** Click the tabs under the mode navigation to switch between different algorithms.
...
## What's Next
The VTable architecture and documentation structs were designed to be generic and algorithm-agnostic, so things you might expect in the future: 
